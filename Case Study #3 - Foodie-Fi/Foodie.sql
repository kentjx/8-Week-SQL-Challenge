...
VALUES
  ...
  -- A. Based off the 8 sample customers provided in the sample from the subscriptions table, 
...
SELECT *, ROUND(CAST(CTE.churned_count as numeric)/ CAST(CTE.total_customer_count as numeric) * 100,2) AS percentage
FROM CTE 

SELECT *, (churned_count/total_customer_count * 100) AS percentage
FROM
(
SELECT COUNT(distinct s.customer_id) AS total_customer_count,
COUNT(distinct CASE WHEN p.plan_id = 4 THEN customer_id ELSE NULL END) AS churned_count
...
SELECT *, ROUND(cast((total_count) as numeric)/1000 * 100, 0) AS percentage
FROM CTE2
...
SELECT plan_id, COUNT(DISTINCT customer_id) AS count_customer
FROM
(
	SELECT s.plan_id, s.customer_id, p.plan_name, LEAD(plan_name, 1) OVER (partition by customer_id order by customer_id, start_date) as subsequent
	FROM subscriptions s 
	LEFT JOIN plans p
	ON s.plan_id = p.plan_id
	ORDER BY s.customer_id, s.start_date
) as alias
WHERE plan_id = 0 AND subsequent IS NOT NULL 
GROUP BY plan_id, subsequent 
...
SELECT subsequent, COUNT(*) AS count_customer, ROUND(COUNT(*):: numeric  * 100 / (SELECT COUNT(distinct customer_id) 
													 FROM subscriptions), 2) 
FROM 
CTE 
WHERE subsequent IS NOT NULL AND plan_id = 0 
GROUP BY 1
