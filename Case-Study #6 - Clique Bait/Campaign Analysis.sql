...
-- looking at count_visits, page_views_avg, and cart_adds_avg, avg_purchase, we can tell that when there's an impression, these business metrics tend to perform better. 
...
-- Looking at the avg_purchase, clicking on impression lead do leads to higher purchase rates 


--Break down the visit start time into year, month, day and hour. Can we identify which month/day/hour have higher purchases rates - whether there is campaign or not. 
...
--inner join clique_bait.events e on c.visit_id = e.visit_id
...
-- In this analysis, the campaign_days indicates the duration of the respective campaigns, each individual product is then summed divided by the length of the of campaign days, and grouped 
-- according to their campaigns names. The value will provide an indication if that particular product undergo an increase in purchases (the CTEs above are filtered to purchases = 1) against
-- the benchmark where there is no campaign (campaign_name = null). Overall, we can tell that campaigns do have an effect on all the products, even for targeted campaigns, they tend to drive 
-- more purchases even from other category. The best performed campaign is Half-Off - Treat Your Shellf(ish), possibly due to the large discounts provided. 
-- One point to note is that salmon has particularly performed poorly and could be a choice to abandon, if needed. 
